        ...
        for case in os.listdir(data_dir): 
            if i == 0:
                if count > 24:
                    ...
                    train_labels.append(os.path.join(data_dir,case,'segmentation_no_infection.nii.gz'))
                    count += 1
                else:
                    val_images.append(os.path.join(data_dir,case,'image.nii.gz'))
                    val_labels.append(os.path.join(data_dir,case,'segmentation_no_infection.nii.gz'))
                    count += 1
            elif i == 1:
                if count < 25 or count > 48:
                    train_images.append(os.path.join(data_dir,case,'image.nii.gz'))
                    train_labels.append(os.path.join(data_dir,case,'segmentation_no_infection.nii.gz'))
                    count += 1
                else:
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    if metric > best_metric:
                        ...
                        torch.save(model.state_dict(), '/home/marafath/scratch/saved_models/eu_model_f{}.pth'.format(i))
