...
class NiftiDataset2(Dataset, Randomizable):
    """
    ...
    def __init__(
        ...
        patient_data: Optional[Sequence[str]] =None,
        labels=None,
        ...
        if patient_data is not None and len(image_files) != len(patient_data):
            raise ValueError("Must have same number of image and patient data files")    
    ...
    def __getitem__(self, index: int):
        ...
        if self.patient_data is not None:
            pat_data_loader = LoadPatientData()
            pat_data = pat_data_loader(self.patient_data[index])
        label = None
...
class LoadPatientData(Transform):
    ...
    def __call__(self, filename):
        ...
        for name in filename:
            img = np.load(name)
            img = img[-107:]
            data = np.asarray(img,np.float64)
